...
def roc_date_input(label, default_date=None, key_suffix=""):
    ...
    y_list = list(range(90, 131))  # 90~130
...
def page_add():
    ...
    if st.session_state["temp_children"]:
        ...
        temp_df = pd.DataFrame(st.session_state["temp_children"]).copy()

        need_cols = ["幼兒姓名", "幼兒生日", "報名狀態", "預計入學資訊", "重要性", "備註"]
        ...
        # 回寫
        edited2 = edited.copy()
        edited2["__刪除__"] = edited2["__刪除__"].fillna(False)
        edited2 = edited2.loc[~edited2["__刪除__"]].drop(columns=["__刪除__"], errors="ignore")
        edited2 = edited2.fillna("").astype(str)
...
def page_manage(df: pd.DataFrame):
    ...
    def process_save_status(tdf: pd.DataFrame, key_pfx: str):
        ...
        for _, r in tdf.iterrows():
            ...
            def set_if_changed(col, val):
                nonlocal changes_made
                if _safe_str(fulldf.at[oid, col]) != _safe_str(val):
                    fulldf.at[oid, col] = _safe_str(val)
                    changes_made = True

            set_if_changed("幼兒姓名", new_name)
            set_if_changed("幼兒生日", new_dob)
            set_if_changed("家長稱呼", new_pname)
            set_if_changed("電話", new_phone)

            if new_contact is not None:
                ncon_str = "已聯繫" if bool(new_contact) else "未聯繫"
                set_if_changed("聯繫狀態", ncon_str)

            if new_status:
                set_if_changed("報名狀態", new_status)

            if new_plan:
                set_if_changed("預計入學資訊", new_plan)

            set_if_changed("備註", new_note)

            if new_imp not in ["優", "中", "差"]:
                new_imp = "中"
            set_if_changed("重要性", new_imp)
...
# Page: 未來入學預覽
# ==========================================
def page_preview(df: pd.DataFrame):
    st.header("📅 未來入學名單預覽")
